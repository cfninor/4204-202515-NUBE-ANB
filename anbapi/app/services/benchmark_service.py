...
class BenchmarkService:
    ...
    def run_saturation_test(self, video_size_mb: int, num_workers: int, num_tasks: int):
        ...
        benchmark_id = f"saturation_{video_size_mb}mb_{num_workers}workers_{int(time.time())}"
    ...
    def _run_saturation_benchmark(self, benchmark_id: str, video_size_mb: int, num_tasks: int):
        """Ejecuta el benchmark de saturación en segundo plano"""
        try:
            print(f"🎬 Iniciando benchmark {benchmark_id} con {num_tasks} tareas de {video_size_mb}MB")
            
            # Inyectar mensajes
            result = self.producer.inject_messages_directly(video_size_mb, num_tasks)
            
            print(f"✅ Mensajes inyectados: {result['total_messages']} en {result['injection_time_seconds']:.2f}s")
            print(f"📊 Tasa de inyección: {result['injection_rate_msg_per_sec']:.2f} mensajes/segundo")
    ...
    def run_sustained_test(self, video_size_mb: int, num_workers: int, queue_size: int, duration_seconds: int):
        """Ejecuta prueba sostenida"""
        benchmark_id = f"sustained_{video_size_mb}mb_{num_workers}workers_{int(time.time())}"
    ...
    def _run_sustained_benchmark(self, benchmark_id: str, video_size_mb: int, queue_size: int, duration_seconds: int):
        ...
        try:
            ...
            print(f"   Duración: {duration_seconds}s, Tamaño cola: {queue_size}, Video: {video_size_mb}MB")
            ...
            while time.time() - start_time < duration_seconds:
                ...
                with self.producer.get_db_session() as db:
                    current_pending = db.execute(
                        select(func.count(Video.id)).where(Video.status == VideoStatus.UPLOADED)
                    ).scalar()
                    
                    tasks_to_add = max(0, queue_size - current_pending)
                    if tasks_to_add > 0:
                        # Inyectar mensajes (esto ya maneja su propia sesión)
                        result = self.producer.inject_messages_directly(video_size_mb, tasks_to_add)
                        all_video_ids.extend(result["video_ids"])
                        print(f"➕ Añadidas {tasks_to_add} tareas, total acumulado: {len(all_video_ids)}")
    ...
    def get_benchmark_status(self, benchmark_id: str, db: Session):
        ...
        if benchmark["status"] == "completed":
            ...
            print(f"Calculando métricas para benchmark {benchmark_id} con {len(video_ids)} videos")
            if video_ids:
    ...
    def _calculate_metrics(self, video_ids: list, db: Session):
        ...
        try:
            ...
            return {
                ...
                "processing_times_sample": [round(t, 2) for t in processing_times[:3]] if processing_times else [],
