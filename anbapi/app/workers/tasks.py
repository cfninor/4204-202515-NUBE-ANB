        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if has_audio_stream(src):
                ...
                run([
                    FFMPEG, "-y",
                    "-i", src,  # [0]
                    "-f", "lavfi", "-t", f"{main_len}", "-i", ANULL,  # [1]
                    "-t", f"{main_len}",
                    "-vf", vf_main,
                    "-r", "30",
                    "-map", "0:v:0", "-map", "1:a:0",
            ...
            if has_logo:
                run([
                    FFMPEG, "-y",
                    "-loop", "1", "-t", f"{intro_len}", "-i", logo,        
                    "-f", "lavfi", "-t", f"{intro_len}", "-i", COLOR,      
                    "-f", "lavfi", "-t", f"{intro_len}", "-i", ANULL,     
                    "-filter_complex",
                    "[0:v]scale='min(720,iw)':'min(300,ih)':force_original_aspect_ratio=decrease,"
                    "format=rgba,fade=t=in:st=0:d=0.5,fade=t=out:st="
                    + str(max(0.0, intro_len - 0.5)) + ":d=0.5[lg];"
            ...
            if has_logo:
                run([
                    FFMPEG, "-y",
                    "-loop", "1", "-t", f"{outro_len}", "-i", logo,        
                    "-f", "lavfi", "-t", f"{outro_len}", "-i", COLOR,      
                    "-f", "lavfi", "-t", f"{outro_len}", "-i", ANULL,     
                    "-filter_complex",
                    "[0:v]scale='min(720,iw)':'min(300,ih)':force_original_aspect_ratio=decrease,"
                    "format=rgba,fade=t=in:st=0:d=0.5,fade=t=out:st="
                    + str(max(0.0, outro_len - 0.5)) + ":d=0.5[lg];"
                    "[1:v][lg]overlay=(W-w)/2:(H-h)/2[v]",
                    "-map", "[v]", "-map", "2:a",
                    "-c:v", "libx264", "-preset", "veryfast", "-crf", "23", "-pix_fmt", "yuv420p",
                    "-c:a", "aac", "-b:a", "160k", "-ar", "48000",
