...
class BenchmarkProducer:
    ...
    def create_real_dummy_video(self, size_mb: int, duration: int = 10) -> str:
        ...
        try:
            ...
            cmd = [
                'ffmpeg', '-y',
                '-f', 'lavfi', '-i', f'color=red:size=640x360:rate=30:d={duration}',
    ...
    def get_base_test_video(self) -> str:
        ...
        if not os.path.exists(base_path):
            ...
            cmd = [
                'ffmpeg', '-y',
                '-f', 'lavfi', '-i', 'color=red:size=640x360:rate=1:d=1',
                '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '30',
                '-t', '1',
                base_path
            ]
    ...
    def expand_video_size(self, input_path: str, output_path: str, target_size_mb: int):
        ...
        try:
            ...
            cmd = [
                'ffmpeg', '-y',
                '-stream_loop', str(target_duration - 1),  # Loop para alcanzar duración
                '-i', input_path,
                '-c', 'copy',
    ...
    def inject_messages_directly(self, video_size_mb: int, num_messages: int):
        ...
        for i in range(num_messages):
            ...
            video_id = self.create_real_dummy_video(
                video_size_mb
            )
